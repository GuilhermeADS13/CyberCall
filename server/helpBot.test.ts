...
import { buildHelpBotMessages, cyberCallHelpSystemPrompt, cyberCallHelpTopics } from "./helpBot";
...
describe("CyberCall help bot", () => {
  it("keeps the assistant scoped to safe CyberCall FAQ topics", () => {
    ...
    expect(cyberCallHelpSystemPrompt).toContain("Não desative nem recomende contornar bloqueios");
  });

  it("prepends policy context and keeps only the latest messages", () => {
    const messages = Array.from({ length: 10 }, (_, index) => ({ role: "user" as const, content: `dúvida ${index}` }));

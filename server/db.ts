...
export async function listMessages(channelId: number, viewerId = 0) {
  ...
  // Single lookup for every reaction in the page; one query per message does not scale.
  ...
  const reactorsByMessage = new Map<number, number[]>();
  for (const reaction of reactionRows) {
    const current = reactorsByMessage.get(reaction.messageId);
    if (current) current.push(reaction.userId);
    else reactorsByMessage.set(reaction.messageId, [reaction.userId]);
  }
  return rows.map(row => {
    const reactors = reactorsByMessage.get(row.id) ?? [];
    return {
      ...row,
...
export async function createCommunity(
  ...
  const communityResult = await db
    .insert(communities)
    .values({ ownerId, name, slug, description: description || null })
    .$returningId();
  ...
  await db.insert(channels).values([
    { communityId, name: "rules", channelType: "announcement", position: 0 },
    { communityId, name: "general", channelType: "text", position: 1 },
    { communityId, name: "lobby", channelType: "voice", position: 2 },
  ]);
...
export async function createAttachment(
  ownerId: number,
  attachment: Omit<AttachmentMeta, "id">
) {
  const db = await getDb();
  if (!db) throw new Error("Database unavailable");
  const result = await db
...
export async function updateMessage(
  ...
  const existing = await db
    ...
    .where(eq(messages.id, messageId))
    .limit(1);
  if (!existing[0] || existing[0].authorId !== authorId)
...
export async function listDirectMessages(userId: number, otherUserId: number) {
  ...
  return db
    .select({
      ...
      attachmentKey: directMessages.attachmentKey,
      attachmentUrl: directMessages.attachmentUrl,
      attachmentName: directMessages.attachmentName,
      attachmentMimeType: directMessages.attachmentMimeType,
      attachmentSize: directMessages.attachmentSize,
...
export async function createDirectMessage(
  ...
  await db.insert(notifications).values({
    userId: recipientId,
    kind: "direct_message",
    title: "Novo sinal privado",
    body: "Você recebeu uma nova mensagem direta na Cyperpuck.",
  });
  return (
    await db
      .select({
...
export async function createRoomInvite(
  ...
  await db.insert(notifications).values({
    userId: recipientId,
    kind: "room_invite",
    title: "Convite de sala recebido",
    body: `Você foi convidado para entrar na sala ${roomName}.`,
  });
...
export async function listRoomInvites(userId: number) {
  ...
  const rows = await db
    .select({
      id: roomInvites.id,
      roomKey: roomInvites.roomKey,
      roomName: roomInvites.roomName,
    ...
    .from(roomInvites)
    .innerJoin(users, eq(users.id, roomInvites.senderId))
    .where(
      and(
        eq(roomInvites.recipientId, userId),
...
export async function respondRoomInvite(
  ...
  const existing = await db
    ...
    .from(roomInvites)
    .where(
      and(eq(roomInvites.id, inviteId), eq(roomInvites.recipientId, userId))
    )
    .limit(1);
  if (!existing[0]) throw new Error("Room invite not found");
  if (existing[0].status !== "pending")
    throw new Error("Room invite already answered");
  ...
  await db.insert(notifications).values({
    ...
    kind: "room_invite_response",
    title: status === "accepted" ? "Convite aceito" : "Convite recusado",
    body:
      status === "accepted"
        ? "Seu convite de sala foi aceito."
        : "Seu convite de sala foi recusado.",
  });

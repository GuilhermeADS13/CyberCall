...
export default function Auth() {
  ...
  function handleTabKeyDown(event: KeyboardEvent<HTMLButtonElement>, index: number) {
    ...
    const nextIndex = event.key === "Home" ? 0 : event.key === "End" ? authModes.length - 1 : (index + (event.key === "ArrowRight" ? 1 : -1) + authModes.length) % authModes.length;
    ...
    <main className="relative min-h-screen overflow-hidden bg-[#080b10] text-[#f3f7f5] selection:bg-[#6fffe9] selection:text-[#080b10]">
      ...
      <header className="relative flex items-center justify-between border-b border-[#26363a] px-5 py-4 sm:px-8">
        <Link href="/" className="flex items-center gap-3 text-[#f3f7f5] transition hover:text-[#6fffe9]">
          <span className="grid h-9 w-9 place-items-center border border-[#6fffe9]/60 text-[#6fffe9]">◈</span>
          <span><strong className="block font-display text-sm tracking-[0.16em]">CYPERPUCK</strong><span className="font-display text-[8px] uppercase tracking-[0.2em] text-[#718183]">Identity gateway / 01</span></span>
        </Link>
      ...
      <div className="relative mx-auto grid min-h-[calc(100vh-73px)] max-w-6xl items-center gap-10 px-5 py-10 sm:px-8 lg:grid-cols-[1fr_460px] lg:gap-20 lg:py-16">
        <section className="max-w-xl">
          ...
          <p className="mt-6 max-w-lg text-base leading-7 text-[#8c9c9e]">Uma identidade para conversar, criar e organizar esquadrões no universo Cyperpuck. Entre com segurança e retome sua arena de onde parou.</p>
        ...
        <section className="border border-[#26363a] bg-[linear-gradient(145deg,rgba(17,26,31,.98),rgba(8,11,16,.98))] p-1 shadow-[0_0_45px_rgba(111,255,233,.06)]" aria-labelledby="auth-title">
          <div className="border border-[#26363a]/70 p-5 sm:p-7">
            {isAuthenticated ? <div className="py-8 text-center"><div className="mx-auto grid h-14 w-14 place-items-center border border-[#6fffe9] bg-[#6fffe9]/10 text-[#6fffe9]"><UserRound size={24} /></div><p className="mt-5 font-display text-[10px] uppercase tracking-[0.2em] text-[#6fffe9]">Identidade reconhecida</p><h1 id="auth-title" className="mt-2 font-display text-2xl tracking-[-0.04em]">Olá, {user?.name || "piloto"}.</h1><p className="mt-3 text-sm leading-6 text-[#8c9c9e]">Sua sessão já está ativa. Volte à arena para continuar.</p><Link href="/" className="mt-7 inline-flex items-center gap-2 bg-[#6fffe9] px-4 py-3 font-display text-[10px] font-bold uppercase tracking-[0.14em] text-[#080b10] transition hover:bg-[#f3f7f5]">Abrir arena <ArrowUpRight size={14} /></Link></div> : <>
              ...
              <div id="auth-panel" className="mt-7" role="tabpanel" aria-labelledby={`auth-tab-${mode}`} aria-live="polite">
                ...
                <button type="button" onClick={startLogin} className="mt-7 flex w-full items-center justify-center gap-3 bg-[#6fffe9] px-4 py-4 font-display text-[10px] font-bold uppercase tracking-[0.16em] text-[#080b10] transition hover:bg-[#f3f7f5] active:scale-[0.98]"><LockKeyhole size={16} /> {authCopy[mode].cta}<ArrowUpRight size={15} /></button>
                <div className="mt-5 flex items-start gap-3 border-l border-[#6fffe9] bg-[#101c20] p-3"><ShieldCheck size={16} className="mt-0.5 shrink-0 text-[#6fffe9]" /><p className="text-xs leading-5 text-[#8c9c9e]">Sem senha local para vazar. O gateway autentica sua sessão e protege o retorno para a Cyperpuck.</p></div>

...
describe("WebRTC mesh signaling", () => {
  it("classifies quality from latency, packet loss and connection state", () => {
    expect(
      classifyNetworkQuality({ rttMs: 80, lossRate: 0.01, connected: true })
    ).toBe("good");
    expect(
      classifyNetworkQuality({ rttMs: 220, lossRate: 0.04, connected: true })
    ).toBe("unstable");
    expect(
      classifyNetworkQuality({ rttMs: 420, lossRate: 0.12, connected: true })
    ).toBe("poor");
  ...
  it("returns explicit media constraints", () => {
    expect(getMediaConstraints(true, false)).toEqual({
      audio: true,
      video: false,
    });
    expect(getMediaConstraints(false, true)).toEqual({
      audio: false,
      video: true,
    });
    expect(
      getMediaConstraints(true, true, {
        audioDeviceId: "mic-1",
        videoDeviceId: "cam-2",
      })
    ).toEqual({
  ...
  it("creates offers for existing members and handles answer/ICE events", async () => {
    const previous = (
      globalThis as typeof globalThis & { RTCPeerConnection?: unknown }
    ).RTCPeerConnection;
    ...
    try {
      const mesh = createWebRtcMesh({
        ...
        roomKey: "lobby",
        localUserId: 7,
        sendSignal: command => commands.push(command),
      });
      await mesh.handleEvent({
        type: "voice.members",
        payload: { members: [{ userId: 8, name: "Pilot 8" }] },
      });
      ...
      await mesh.handleEvent({
        type: "voice.answer",
        payload: { fromUserId: 8, sdp: { type: "answer", sdp: "answer-sdp" } },
      });
      await mesh.handleEvent({
        type: "voice.ice",
        payload: { fromUserId: 8, candidate: { candidate: "candidate:1" } },
      });
      expect(FakePeerConnection.instances[0].remoteDescriptions).toHaveLength(
        1
      );
      expect(FakePeerConnection.instances[0].candidates).toEqual([
...
describe("ICE configuration", () => {
  it("falls back to public STUN when nothing is configured", () => {
    ...
    expect(hasTurnRelay(configuration)).toBe(false);
  });

  it("adds the TURN relay with its credentials when configured", () => {
    const configuration = buildRtcConfiguration({
      VITE_STUN_URLS: "stun:stun.example:3478",
      VITE_TURN_URLS: "turn:relay.example:3478, turns:relay.example:5349",
      VITE_TURN_USERNAME: "piloto",
    ...
    expect(hasTurnRelay(configuration)).toBe(true);
  });

  it("ignores blank entries in the server lists", () => {
    const configuration = buildRtcConfiguration({
      VITE_STUN_URLS: " , ",
      VITE_TURN_URLS: "",
    });

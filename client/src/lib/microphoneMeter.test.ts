...
describe("microphone meter", () => {
  ...
  it("reports unsupported runtimes without throwing", () => {
    const meter = createMicrophoneMeter({} as MediaStream, vi.fn(), {} as typeof globalThis);
  ...
  it("connects an analyser, emits a level and closes resources", async () => {
    ...
    const analyser = { fftSize: 0, getByteTimeDomainData: (samples: Uint8Array) => samples.fill(160), disconnect: disconnectAnalyser };
    const runtime = {
      AudioContext: class {
    ...
    const levels: number[] = [];
    const meter = createMicrophoneMeter({} as MediaStream, (level) => levels.push(level), runtime);
